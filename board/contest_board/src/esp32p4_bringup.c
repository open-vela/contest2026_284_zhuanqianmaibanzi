...
#  endif /* CONFIG_ESPRESSIF_SPI_BITBANG */
  ...
  /* Product-specific control GPIOs, status device and GT911 are initialized
   * after I2C registration.  A failure is reported but does not remove the
   * UART/NSH recovery path.
   */

  ret = velapoka_bsp_initialize();
  if (ret < 0)
    {
